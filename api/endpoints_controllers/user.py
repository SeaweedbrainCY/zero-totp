...
def update_vault(user_id, body):
    ...
    for secret in old_vault:
        if secret.uuid not in uuids_from_new_vault:
            logging.warning(f"Error. The user {user_id} tried to update but the secret {secret.uuid} was not sent in his new vault. This means the secret would not have been updated. The update is rejected.")
    ...
    try:
        UpdatePassphraseRepo().updatePassphrase(user_id=user_id, newPassphrase=hashedpw, newZKEKey=zke_key, newPassphraseSalt=passphrase_salt, newDerivedKeySalt=derivedKeySalt, newVault=enc_vault)
    except Exception as e:
        logging.error(f"An error occured while updating the passphrase of user {user_id}. Error: {e}")
