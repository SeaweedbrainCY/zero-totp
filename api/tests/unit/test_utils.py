...
##################
    ...
    def test_get_rich_geolocation(self):
        ip = "157.159.40.44"
        geolocation = get_geolocation(ip)
        self.assertIn(geolocation, ["157.159.40.44 (France)", "157.159.40.44 (Évry, Essonne, France)", "157.159.40.44 (Évry-Courcouronnes, Essonne, France)"])
...
#########
    ...
    def test_get_forwarded_for_with_trusted_proxy_ipv6(self):
        ...
        request.headers = {"X-Forwarded-For": "2001:4860:4860:0:0:0:0:8888"}
        conf.api.trusted_proxy = [ipaddress.ip_network("2001:db8::/32")]
        ip = get_ip(request)
        self.assertEqual(ip, "2001:4860:4860:0:0:0:0:8888")
        conf.api.trusted_proxy = None
